...
core_update ()
{
    ...
    if [[ "$remote_version" == "$local_version" ]]; then
        ...
        if [[ -z "$choice" || "$choice" =~ ^(yes|y|Y) ]]; then
            ...
            check_for_modifications "${CORE_CONFIG}/plugins.js" "${CORE_DIR}/packages/core/lib/config/${CORE_NETWORK}/plugins.js"
            check_for_modifications "${CORE_CONFIG}/network.json" "${CORE_DIR}/packages/crypto/lib/networks/${CORE_TOKEN}/${CORE_NETWORK}.json"
