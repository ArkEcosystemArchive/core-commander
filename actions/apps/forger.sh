...
forger_restart ()
{
    ...
    pm2 restart ark-core-forger | tee -a "$commander_log"
...
forger_stop ()
{
    ...
    pm2 stop ark-core-forger | tee -a "$commander_log"
...
forger_delete ()
{
    ...
    pm2 delete ark-core-forger | tee -a "$commander_log"
...
__forger_configure_plain ()
{
    ...
    if [[ $status -ne 0 ]]; then
        error "Sorry, an unknown error occurred. Please try again." | tee -a "$commander_log"
    else
...
__forger_start_with_bip38 ()
{
    ...
    pm2 start "$CORE_DIR/packages/core/bin/ark" --name ark-core-forger -- forger --data "$CORE_DATA" --config "$CORE_CONFIG" --token "$CORE_TOKEN" --network "$CORE_NETWORK" --bip38 "$bip38" --password "$password" | tee -a "$commander_log"
}

__forger_start_without_bip38 ()
{
    pm2 start "$CORE_DIR/packages/core/bin/ark" --name ark-core-forger -- forger --data "$CORE_DATA" --config "$CORE_CONFIG" --token "$CORE_TOKEN" --network "$CORE_NETWORK" | tee -a "$commander_log"
}
